...
def load_data_into_vectorstore(client, docs: List[str], openai_api_key: str):
...
def main():
    ...
    with st.sidebar:
        openai_api_key = st.text_input(label='#### OpenAI API 키', placeholder="OpenAI API 키를 입력하세요", type="password")
        notion_api_key = st.text_input(label='#### Notion API 키', placeholder="Notion API 키를 입력하세요", type="password")
